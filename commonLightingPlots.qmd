...
These diagrams are approximations (dimension wise), and somewhat simplified. For example, the stage has some rounded corners at the front which are not depicted, and the physical stage is not symmetrical. 
...
Also excluded are two (black) incandescent fixtures which are hanging from the balcony bars. 

Some items are quite moveable so may not match this diagram, specifically fixtures 34-40 and 67.

* 34-40 are sometimes free standing and washing the walls around the screens, backlight for the scrims or installed inside the white plastic pillars.
* 67 (which is actually 6 bars) has been used for a ceiling wash, the large scrims,  etc.

Zoom in on the stage (@fig-clpzoomstage). Some of the lighting is mounted on the ceiling and some sits on the stage. Some of them are portable (CSPARs and AW Bars) and are used for lighting scrims, backdrops, etc.
...
* Each main house pendant has two groups of lamps. An outer group of four lamps and an inner single lamp . The four outer lamps have a wide beam spread, the inner one is narrow. They are colour controllable (LED RGB).
...
:::{.callout-note}
There are 20 pendants currently installed. There were originally 24. There were four pendants in the vacinity of the balcony bars that were removed to make room for the bars. Two over the balcony and two over the main floor.  
:::

```{r  }
#| label: tbl-clppendants
#| tbl-cap: Pendants
...
:::{.callout-note}
The sconce lights will likely be replaced by white LEDs and and no longer DMX controlled at some point in the future.
:::
...
The plot is illustrative - They are not accurately placed. There are 26 pot lights (all Ketra). 

```{r   }
#| label: tbl-clpubal
#| tbl-cap: Under Balcony
#| tbl-cap-location: bottom
