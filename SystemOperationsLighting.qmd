...
#. The cue master must be up See @fig-sol-cue-master
#. CueCommanderNR must be running,  see @sec-cuecommander 

![Cue Master](./images/sol-cue-master.png){#fig-sol-cue-master width=33%}

<!--
Photoshop source file: ... SystemDocumentation/github/uactechdoc/images/clku-c001-display.psd
-->
...
#| label: tbl-playbacks#| tbl-cap: Playback Map#| tbl-cap-location: bottom
...
:::{.callout-note}
TODO: **The list below is out of date and needs to be updated.** See @fig-intermediate-playbacks
:::

```{r intermediate}
#| label: tbl-intermediate-playbacks
playback %>%
  filter(IsSimple == "N"  ) %>%
#  arrange(Playback) %>%
  select(-IsSimple) %>%
...
```{r tab-intermediate}
#| tbl-cap: "Intermediate Training"
#| tbl-cap-location: bottom
#| label: tbl-intermendiate
#| 
get.training() %>%
  filter(Topic1=="Lighting") %>%
  filter(Topic2=="Console Operations") %>%
...
:::{.callout-note}
TODO Remove the NAs and add links in @tag-design-training
:::
