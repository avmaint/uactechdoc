...
The video mixer will likely have been turned on by the audio engineer.
If by chance the Propresenter computer was awoke before you will not see its video feed.
See the troubleshooting section for corrective steps.
...
6.  
    ...
    :   The whole team of 6 or so singers.
        When used you may need to go wider of narrower depending on the vocal compliment of that week.
...
db.cables |> 
	...
	left_join(db.assets, join_by (SrcTag == AssetTag )) |>
	select(DstPort, Usage.x ,  SrcTag, SrcPort, Desc  ) |> 
	arrange(DstPort) |> 
	gt() |>
	opt_stylize(style=3) |>
	...
	tab_footnote("Usually used for the Stage camera via extension cable"
				 , locations= 
				 	cells_body(columns=DstPort,
				 						rows = DstPort == 'in8'))
```

 

In addition to the eight physical inputs, there are several more internal inputs.
This is the complete list and the associated key combination for that source.

```{r}
srcs <- tribble(
	~Source, ~KeyCombo, ~Desc
, "Input 1", "1", "ProPresenter SW Image"
, "Input 2", "2", "ProPresenter Key"
, "Input 3", "3", "From Matrix"
, "Input 4", "4", "ProPresenter Fill"
, "Input 5", "5", "Camera 1"
, "Input 6", "6", "Camera 2"
, "Input 7", "7", "Camera 3"
, "Input 8", "8", "Stage Camera"
, "Media Player 1", "Shift-1", ""
, "Media Player 1 Key", "Shift-2", ""
, "Media Player 2", "Shift-3", ""
, "Media Player 2 Key", "Shift-4", ""
, "Background Colour 1", "Shift-5", ""
, "Background Colour 2", "Shift-6", ""
...
(See @tab-atem-outputs / @fig-atem-outputs)
...
db.cables |> 
	...
	left_join(db.assets, join_by (DstTag == AssetTag )) |>
	select(SrcPort, Usage.x ,  DstTag, DstPort, Desc  ) |> 
	arrange(SrcPort) |> 
	gt() |>
	opt_stylize(style=3) |>
...
ProPresenter sends two video signals (key and fill) from ProPresenter which allows text, graphic or video content to partially overlay the camera image.
...
:   This is the the text or graphic from ProPresenter which we want to show on top of the camera image.
...
## 5.x Using the Program Bus

This section needs to move down

There are several sections that we will use a lot.

Program and Preview buttons (@fig-sov-ProgramPreview)

:   In the lower left corner there are two rows of 8 buttons.
    They are not labelled as Program and Preview, but instead with the corresponding source.
...
If they are flashing then you are using the shifted source If they are blue, then you are using the double shifted source
:::

![Program and Preview, Lower Left](./images/sov-ProgramPreview.jpg){#fig-sov-ProgramPreview}

CUT and AUTO Buttons (Next to Slider) @fig-sov-CutAuto
...
![Cut and Auto, Lower Centre](./images/sov-CutAuto.jpg){#fig-sov-CutAuto width="50%"}
...
![Mix, Top Centre](./images/sov-Mix.jpg){#fig-sov-Mix width="50%"}

DVE PiP Buttons (@fig-sov-PiP)
...
![DVE PiP, Right Middle](./images/sov-PiP.jpg){#fig-sov-PiP width="50%"}

Key 1 Fill (@fig-sov-Key1Fill)

:   Left side, middle

-   This button is used in conjunction with the numbered keys immediately below to select the source to be placed in a PiP.

![Key 1 Fill, Left Middle](./images/sov-Key1Fill.jpg){#fig-sov-Key1Fill width="50%"}

::: callout-note
TODO - add more content about USK and DSK
:::
...
::: callout-tip
Note you can also perform direct cut using the red buttons.
:::
...
::: callout-tip
The Picture in Picture (PiP) feature uses the Upstrem keyer.
If you have set the USK to use as a alpha key for lyrics, it will have to be reset to Luma after using PiP.
:::

::: callout-note
TODO To turn PiP off ... I don't remember - fiddle with the BKGD and "Key 1 TIE" buttons.
:::
...
Downstream Key is abbreviated as DSK.
:::

::: callout-note
There is a dedicated word document on this topic that goes into great detail.
See "deepdive_livestream_graphics.docx"
:::
...
The ATEM mixer has two built-in media players which can play images and very short videos.
...
A macro is an easy way to automate a sequence of switcher actions so you can repeat the sequence at the press of a button.
This makes the execution of a complex set of steps easy to do in a hurray and without error.

### Running Macros

There are three ways to run macros: From the panel or via the control software.

**Running Macros from the panel**

1.  Press the macro button and then the number of the macro you wish to execute

**Running Macros from the ATEM Control Software**

1.  On the Video Mac (CUMU-G001), Open the application (if not already running)
...
macros <- tribble(
~Macro, ~Type, ~Desc  
, 1, "Do", "Setup pre-service"
, 3, "Do", "Start Service" 
, 4, "Do", "Stop Service"
, 5, "Do", "Start Message"
, 6, "Do", "Stop Message"

,16, "Do", "Aux1 from ProP"
,17, "Do", "Aux1 from CDWU-0009"
,18, "Do", "Aux1 from Program"
,19, "Do", "USK On - Turn on the Upstream keyer"
,20, "Do", "USK Off - Turn off the Upstream keyer" 

,21, "Setup", "USK for ProP overlay. This sets the upstream key to create a overlay of just the bright parts of the ProPresenter source - which is usually just the Text" 
,22, "Setup", "Pre-Message MP"
,23, "Setup", "DSK1 and DSK2 for luma key"
,24, "Setup", "Post-Message video Trailer MP"
)

steps <- tribble(
~Macro, ~Step, ~Action
, 1, 1, "Run 21 setup USK"
, 1, 2, "Run 22 setup Pre-message MP"
, 1, 3, "Run 23 setup DSK1 and dsk2"

,5,  1, "Run  MP setup 22"
,5,  2, "Run DSK setup 23"
,5,  3, "Run start recorder 2"
,5,  4, "Fire DSK1" 
,5,  5, "pause for 3s"
,5,  6, "remove DSK1, Fire DSK2" 
,5,  7, "Pause for 3s"
,5,  8, "Remove DSK2"

,6,  1, "Set post msg trailer mp1 24"  
,6,  2, "Fire mp1"
,6,  3, "Pause 3s"
,6,  4, "FTB"
,6,  5, "Run stop recorder 2"
,6,  6, "fire camera 2"
,6,  7, "FTB"
,21, 1, "set fill and key to PP. Threshold at 92, gain 70"
,21, 2, "source and key as ProP"
,21, 3, "Set Flying key position A to Bottom Centre x=0, y=-3, size=.8"
,21, 4, "Set Flying key position B to Bottom Right  x=6, y=-4, size=.6" 

,22, 1, "Move graphic 1 to Mp1 (church Logo)"
,22, 2, "Move graphic 2 to MP2 (speaker L3rd)"

,23, 1, "%10, source and key to be respective MP"

,24, 1, "Move Graphic 3 to MP1"
)
 
macros |>
	left_join(steps) |>
	gt(groupname_col = c("Macro", "Type", "Desc")) |>
	opt_stylize(style=3) |>
	tab_style(
...
The video mixer manual is a massive 1673 pages, but don't be scared by that.
It is the same content repeated for 8 different languages and four different products, so that reduces it to about 100 pages of relevant content for our product.
...
Be aware that not all products offer the same feature sets, so you when reading a section you need to watch out for sections they may only apply to the "Broadcast 1 M/E" or "Broadcast 1 M/E" products.
They have more features than ours.
Our video mixer model is called a "Blackmagic Design ATEM Television Studio Pro HD".
...
:   Starts a short section on upstream and downstream keying.
...
:   The Aux button.
    Our product has only one Aux output.
...
:   Keying.
    Pay particular attention to Luma keying on page 136.
    I have found that use of the clip parameter an important function for a good key.

page 157-163

:   Macros.
    I think they are an important feature we can leverage to make our opening much smoother.

Page 33

:   Using the ATEM Control Software.
    Most things can be done directly from the panel, but some things are much easier from the software.
...
These record onto SD memory cards.
You can observe and verify what is being recorded by looking at the mini screens on the device.
If the screen is black you may want to record a few seconds and then stop recording to esnure things are correctly initialized.
...
The amount of recording time shold be check well before the beginnning of service.
...
::: callout-tip
You can also navigate using the arrow keys rather then the jog wheel.
:::
...
If not then hit the 'some button - don't recall which' button on recorder #1 as it is a source for the web presenter.

There is a mini-switcher built into the web presenter.
The box is wired to see the ATEM Program output on #1 and the ATEM Aux output on #2.
You typically will want the #1 for recording or live streaming.

In normal usage we use this device to feed the program feed into OBS on the video computer (CUMU-G001) where OBS sends it out as an NDI video feed to the prayer room.

OBS could also be used for a special event where we want to stream direct to, eg Youtube

The webpresenter can also be used for video calls, see @sec-video-call.

```{r wpcon-gen}
target <- wp <- c('ZVIU-A004', "ZVKU-A003" )

xd <- c("2308-1124","ZVIU-C001","ZVIU-E003","ZVIU-A008"
		, "ZVKU-A001"
		,"ZVCU-A001","ZVCU-A002", "ZVCU-A003"
		, "2307-2306", "2308-1123", "ZVIU-D002"
		, "ZLIU-A002", "ZAMU-A001", "ZAMU-A002", "ZAMUB001"
		, "ZVVU-0002", "ZVIU-A001", "ZVIU-A002"
		, "CDMU-A001", "CDWU-0009")
diag <- get_diagram(target , db.assets, db.cables
					, exc_dev =xd
					, label= '"Web Presenter Connections"'
					, rankdir ="TB")

gv_path  <- "_work/diag598.gv"
png_path <- "_work/diag598.png"
write_file(diag, gv_path)
grViz(diag) |> export_svg() |> charToRaw() |> rsvg_png(png_path, width = 1200)
```

::: {.content-visible when-format="html"}
```{dot  }
//| fig-cap: Web Presenter Connections
//| cap-location: bottom
//| label: fig-wp-conn
//| file: _work/diag598.gv
```
:::
...
The "Program" output from the ATEM is wired into the input of a "Resi" hardware encoder via SDI.
This device encodes the video stream for internet transmission and sends it to the Resi cloud service.
This service as a standing configuration that broadcasts our service beginning at 9:55 am.
...
The video mixer output is sent to the input of the "Web Presenter" box.
Its output is feed via USB to the videomac (CUMU-G001) computer. The computer sees that devices as a web cam.
...
* Enter the address for a camera in the address bar (see @fig-camera-address). They will be on the left bookmark bar but if not, listed here as well.
...
, "This instructs the camera to make the image a little bit darker than it thinks it should be. We are trying to get good face exposure. This value can later be tweaked using the brigtness knob"
, "Iris" , "**F1.6**"
...
We make two video recordings: 1) for the entire serice, and 2) just the message portion.
...
There are four tabs which we will use along the botttom of the screen.
...
2.  Check the beginning and end of the recording to make sure it starts and ends well.
    If there are any corrections to make, eg trimming the end of the video becuase we didn't end too soon, do that now.
...
-   Sometimes a camera gets confused about what to focus on.
    ...
    We should be able to use auto-focus most of the time, but when the camera gets it wrong, it can be helped by by switching to manual focus (for a time).
...
**Don't see lyric overlay as expected (when pressing "Key 1 Cut")**
