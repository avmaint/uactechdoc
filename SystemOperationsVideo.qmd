...
If by chance the ProPresenter computer was woken before the video mixer, you will not see its video feed.
...
The complete list of external sources is in @tbl-input-map:
...
#| tbl-cap: ATEM Sources and Key Combinations
srcs <- tribble(
...
(See @tbl-atem-outputs and @fig-atem-outputs)
...
-   The Web Presenter device (ZVIU-A004), which is connected to the VideoMac (CUMU-G001) where it can provide video input for Zoom or OBS.

The Auxiliary output is sent to two destinations:

-   The video matrix (ZVKU-A001), where it can be further routed to any of its outputs.
...
-   Pushing the CUT button will immediately switch the next source (green) to live, and place the one that was live as the next.
-   The AUTO button also swaps current and next but does so using the selected transition effect, such as mix.
...
-   The mix button, when pressed, sets the transition to be a dissolve between the current and next images (one image will fade away as the other fades in).
...
-   You would then press the 'Key 1 Fill' button (on the opposite side of the console) and one of the small numbers underneath to select which source you want to use as the fill.
...
-   Decide on which corner of the screen you want to have the PiP and press the corresponding DVE PiP Button
...
::: callout-tip
The Picture in Picture (PiP) feature uses the Upstream keyer.
If you have set the USK to use as an alpha key for lyrics, it will have to be reset to Luma after using PiP.
:::
...
This makes the execution of a complex set of steps easy to do in a hurry and without error.

### Running Macros

There are two ways to run macros: from the panel or via the control software.
