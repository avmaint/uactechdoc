...
This graphic is chnaged be using the ATEM control software on the Mac (CUMU-G001). 
...
The Vimeo app that is installed on the computer (CUMU-G001) is configured to automatically upload any files added to the "VimeoAutoUpload" folder inside the movies folder.

__Vimeo App__

On the videa mac (CUMU-G001) there is a installed application which is the Vimeo helper. You access it via the menu bar at the top. Look for the Vimeo "V" logo.
...
Open you web browser, go to [Vimeo](http://www.vimeo.com), and logon with userid uacmedia@uachome.org. The password is provided separaetly. Upload the file using one of the above mechanisms. Set the various fields that describe the video:
...
* Assign it to the collection for the series. 
* Update the video thumbnail. Upload the prepared thumbnail from the weekly folder and then select it.

After upload, vimeo takes a while to prepare the video for publication - maybe 10 or 15 minutes.  

## Creating the Podcast Audio file

* On the Mac desktop, there an icon for "Make_Audio", Double click on it. 
* After a moment it will pop a choose file dialogue. Navigate to today's video file, and select it.
* It will take about a minute to process the audio.
* The resulting file will be placed in the same folder.
* It is now ready to be uploaded to Nucleus.
...
Select the "Sermon Engine" tab, you will then see a list of the sermons; the one you want should be at the top. Click on it. 

* Paste in the VIMEO URL copied from above. 

* Upload the audio file, which will also be in the VimeoAutoUpload folder. Slide the "Add to Podcast" button.
