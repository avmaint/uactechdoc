...
The video mixer, VideoHub and Stream Deck are powered all the time. If by chance the ProPresenter computer was woken before the video mixer, you will not see its video feed. See the troubleshooting section for corrective steps.
...
tribble(
	...
	"(none)", "5", "Reserved for a future fifth camera",
...
The Web Presenter is fed the ATEM program by the VideoHub (output 28) and its USB output appears on the video computer (CUMU-G001) as a web cam.
...
-   To fix, reset the output resolution of the ProPresenter Mac (CUMU-G002): pull up the Mac display settings, change the resolution to something else and change it back.
-   The projectors will flicker as they resync and ProPresenter will need to be restarted.

**No video signal from the Web Presenter on the computer**
