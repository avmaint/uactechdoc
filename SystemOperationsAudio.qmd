...
#| label: fig-mixrack-inputs-analog
#| fig-cap: MixRack Analog Inputs
get_inputs("MixRack") |>
   select(Name, Port, `dLive Inputs (normal)`) |>	
  gt() |>
...
#| label: fig-mixrack-inputs-dx168
#| fig-cap: DX168 Drum rack Analog Inputs
get_inputs("DX1-DX168") |>
   select(Name, Port, `dLive Inputs (normal)`) |>	
  gt() |>
...
#| label: fig-mixrack-dante-inputs
#| fig-cap: MixRack Dante Inputs
get_inputs("IO1-Dante") |>
	filter(Port < 20) |>
   select(Name, Device, Port, `dLive Inputs (normal)`) |>	
  gt() |>
...
rbind(o1,o2,o3,o4) |>
	select(-Usage) |>
	pivot_wider(names_from = "Device", values_from = "Name") |>
	gt() |>
...
#| label: fig-mixrack-dante-inputs-vsc
#| fig-cap: Dante Inputs for Virtual Sound Check
get_inputs("IO1-Dante") |>
	filter(Port > "64" ) |>
   select(Name, Device, Port,   `dLive Input (VSC)`) |>	
  gt() |>
...
#| label: fig-s5000-analog-inputs
#| fig-cap: S5000 surface Analog Inputs  
get_inputs("S5000") |> 
  select(Name, Device, Port, `dLive Inputs (normal)`) |>	
  gt() |>
...
```{r}
#| label: fig-soa-dlive-softkeys
#| fig-cap: dLive Sof Keys
softkeys <- tribble(
~SoftKey , ~Function 
