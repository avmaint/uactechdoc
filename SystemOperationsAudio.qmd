...
# Introduction
This document provides basic operating information for the main auditorium audio system at UAC.
...
See @tbl-training-videos for a list of training videos. The titles are links you can follow.
...
There is a good collection of videos on YouTube from the [Dante Certification program](https://www.audinate.com/learning/training-certification/dante-certification-program) . You can enroll in the program which will give you access to all of the videos. The level 1 videos are listed here @tbl-training-videos-dante.
...
__2. DX168 Inputs:__
There are 16 analog inputs in the small stage rack located next to the drum riser. They are used for the drum inputs and the bass. 
...
get_inputs("2407-3002") |>
	filter(readr::parse_number(DstPort) >= 65 ) |>   # port names are inconsistently zero-padded, so compare numerically
...
![Surface Rear (inputs are on the left of the panel)](./images/sda-s5000-rear.jpg){#fig-sda-s5000-rear-inputs width=125mm}
...
__1. DM64 Outputs:__
There are 32 analog outputs on the front face of the DM64 mounted in the amp rack. Outputs 31 and 32 carry an analog copy of the main Left and Right as a backup to the Dante feed.
...
The table below shows the configured Dante outputs from the dLive system.

```{r dlive-dante-outputs}
#| label: tbl-dlive-dante-outputs
#| tbl-cap: dLive Dante Outputs
dlive_devices <- c("2407-3000", "2407-3001", "2407-3002", "2410-1902")
db.cables |>
	filter(Protocol %in% c("dante", "GigaAce"), SrcTag %in% dlive_devices, !is.na(Usage)) |>
	left_join(assets |> select(AssetTag, Model, Desc), by = c("SrcTag" = "AssetTag")) |>
	mutate(Console = paste(SrcTag, coalesce(Model, ""), coalesce(Desc, "")) |> str_squish()) |>
	select(Console, SrcPort, DstTag, DstPort, Usage) |>
	arrange(Console, SrcPort) |>
	gt() |>
	opt_stylize(style = 3) |>
	tab_header("dLive Dante Outputs")
```

 

__4. Surface Outputs:__ 
There are no surface outputs in regular use. If there was a need for an output to send to a videographer or such, these outputs could be used for that, but they are not currently pre-configured for that. (I would likely route the mains in parallel for that scenario)

![Surface Rear (outputs are on the right of the panel)](./images/sda-s5000-rear.jpg){#fig-sda-s5000-rear-outputs width=125mm}

## Monitors

We have eight monitor amplifier channels, each which can handle a load of two of our monitor wedges. 

Our standard monitor configuration is @tbl-monitors, however with our usage of the IEMs, these are not all that often used.
...
![Sub Send](./images/soa_qsys_touch.png){#fig-soa-sub-send width=65mm}
...
![Video Mix](./images/soa_qsys_touch.png){#fig-soa-video-send width=65mm}
...
Four Stereo busses have been allocated for Effects. The master scene configures those as **Plate**, **Hall**, **Chorus**, and **Delay**. Softkeys 21, 22 and 23 pull up the Plate, Hall and Delay sends on faders, and key 20 is the Delay tap-tempo; Chorus does not have a softkey. 
...
When recalling a scene, whatever is being recalled will (almost) instantly become active or inactive. You __don't__ want to do this when live, unless you know exactly what you are doing.

__Scene 001__

To recall Scene 001, press the scene button on the right hand side, and then using the right monitor, scroll to the top of the list where you will find "Master". Touch it to select it and then press the physical "Go" button to the right.   (@fig-soa-scene-recall).

![Scene Recall](./images/soa-dlive-scene-recall.png){#fig-soa-scene-recall width=65mm}

## dLive Softkeys 

The softkeys allow the operator to quickly access common features. They are located on the very right hand side of the console, see figure @fig-soa-s5000-softkeys for the location and  @tbl-soa-dlive-softkeys for the standard usage.

```{r}
#| label: tbl-soa-dlive-softkeys
#| tbl-cap: dLive Soft Keys
softkeys <- tribble(
~SoftKey , ~Function 
...
See @tbl-dlive-dante-outputs for more details.
:::
...
The **CUE** button activates that channel to be heard via the headphones. The volume on the headphones can be adjusted via the monitor level below-right to the screen as well as the knob under the wrist rest in the centre. 
...
To adjust the monitor mix you can press one of the buttons labelled "M1" thru "M6" to the lower-right of the screen. Pressing the same button will revert the faders to the main mix.
...
* Using the phase inversion switch can help when desperate but can cause other weird effects, especially of multiple microphones are in use.
...
2) A particular channel is on and it is causing problems for people - for example, say the preacher has turned their mic on at the beginning of the service and is singing along, and it is also turned up in the IEMs.
...
We have 18 wireless transmitters and 16 receivers. A receiver can only be paired with one transmitter at a time, which means we can have, at most 16 active transmitters.
