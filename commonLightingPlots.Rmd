...
These diagrams are approximations dimension wise, also somewhat simplified. The stage has some rounded corners at the front which are not depcited. As well the physical stage is not quite symmetrical. 

## Theartrical Channels
This first diagram shows full room and all theatrical fixtures, and so excludes the architectural fixtures (i.e. Excludes house lights, stage pots, sconse, house pendants and under balcony pots.)
...
> In October 2018, the groups "Mid Stage" and "Centre1", "Centre2", and "Centre3" were all collapsed in to "Centre" .
...
>TODO: Positions are only notional guesses and not complete. 
...
* These pendants have only two lamps. The two inner pendants have two channels. The diagram exagertes the distance between them so the channel numbers can be read. These two provide lighting for the technical teams for setting up.
...
The plot is illustrative - They are not correctly placed. There are 24 Ketra pot lights. 
