...
* These pendants have only the outer group of lamps. The two inner pendants have two channels. The diagram exagertes the distance between them so the channel numbers can be read.
...
##Sconce (Sconse along the main floor perimeter)

* These lamps are RGB lamps.
* There is also a pair of 3-way light swtiches at the entrance doors that can turn off the lights. These are controlled via a two-way switch located by the south-east and north-east doors. The control at the south-east door has a programmable timer. This is to ensure that there is some lighting in the evenings between dusk and midnight to enable safe passage.

>TODO: Positions are only guesses based on memory. 
