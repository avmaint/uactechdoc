...
>TODO: add inventory items for the FoH Rack
...
#todo apply row groupings on the Console and Slot / direction columnes
links %>%
  kable( ) %>%
...
TODO: add the patch matrix


```{r try , eval=FALSE}
# scratch segemnt to play with column subsetting
a <- c( 1, NA, NA, NA)
b <- c(NA, 1, NA,NA)
c <- c(6, 7, 8, NA)
d <- data.frame(cbind(a,b,c)) 
d
#d %>% select_if(~sum(!is.na(.)) > 0) 
#d[complete.cases(d)]

#d %>% filter_all( all_vars( is.na(.))   )

#d %>% na.omit()

#d[ rowSums(is.na(d)) < length(d),  ]

d %>% mutate(t = rowSums(., na.rm=TRUE))
```
...
#routing$rs <- rowSums(routing, na.rm = TRUE)

routing %>% mutate( rs = rowSums( .[,2:nc] ,na.rm=TRUE)) %>% View()
...
#todo: for printing - remove rows and columns which are all NA 
...
routing %>% #[ rowSums(is.na(routing)) < length(routing)-1,  ]%>%
...
routing[ rowSums(is.na(routing)) < length(routing)-1,  ] %>%
    select("Name",  
      ...
      select_if(~sum(!is.na(.)) > 0) %>% #eliminate columns with no mappings
