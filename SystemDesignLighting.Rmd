...
The lighting console is an ETC Express 125. We use only one DMX universe for output. 

The lighting console provides the primary means of control for all users. At times control can be provided by a computer running software such as QLC+.
...
c(101,145),
"ColorSource Pars Overhead and Points - 5 channels each",
...
```{r}
circuits <- tribble(
  ~CircuitRange, ~Destination,
  c ( 1,10), "Balcony East Bar",
  c(11,20),  "Balcony West Bar", 
  c(21,32),	 "Attic", 
  c(40,42),  "Stage Floor" 
)
...
```{r}
dmx.details <- tribble(
  ~DMX.Num, ~Circuit , ~Purpose,
  1, c(9, 11 ), "?",
  2, c(6, 12 ), "?",
  3, "18",   "Pennisula West Bar",
  4, "5",    "Pennisula East Bar",
  5, c(7,13), "SL+Centre",
  6, "",     "Dimmer 6 seems to have troubles",
  7, "8",	   "Wall West",
  8, "14",   "Wall East",
  9, "4",    "SL",
  10, "1",   "Band SR",
...
seq(29,32), "",  "Scrollers",
seq(33,36), "",  "Scrollers",
c(47,48), "",    "Rotator control/reserved",
c(49,50), "",     "Rotator control/reserved"
)

dmx.details %>% formattable(align="l")
```
...
```{r}
k <- tribble(
~Zones, ~Text, ~Num, ~Red, ~Green, ~Blue,
"Walls","Console",101,101,102,103,
"","DMX",151,151,152,153,
"Down Stage","Console",104,104,105,106,
"","DMX",154,154,155,156,
"Mid Stage","Console",107,107,108,109,
"","DMX",157,157,158,159,
"Up Stage","Console",110,110,111,112,
"","DMX",160,160,161,162,
"Centre 1","Console",113,113,114,115,
"","DMX",163,163,164,165,
"Centre 2","Console",116,116,117,118,
"","DMX",166,166,167,168,
"Centre 1","Console",119,119,120,121,
"","DMX",169,169,170,171
)

k %>% select(-Num) %>% formattable(align="l")
```
...
```{r}
csp <- tribble(
~Fixture, ~Text, ~Patch, ~Intensity, ~Red, ~Green, ~Blue, ~Strobe,
1,"Console",51,51,52,53,54,55,
"","DMX",101,101,102,103,104,105,
 
2,"Console",56,56,57,58,59,60,
"","DMX",106,106,107,108,109,110,

3,"Console",61,61,62,63,64,65,
"","DMX",111,111,112,113,114,115,

4,"Console",66,66,67,68,69,70,
"","DMX",116,116,117,118,119,120,

5,"Console",71,71,72,73,74,75,
"","DMX",121,121,122,123,124,125,

6,"Console",76,76,77,78,79,80,
"","DMX",126,126,127,128,129,130,

7,"Console",81,81,81,83,84,85,
"","DMX",131,131,132,133,134,135,

8,"Console",86,86,87,88,89,90,
"","DMX",136,136,137,138,139,140,

9,"Console",91,91,92,93,94,95,
"","DMX",141,141,142,143,144,145
)

csp %>% select(-Patch) %>% formattable(align="l")
```
...
* N is the number of (DMX) Channels per Group 
* G is the number of groups
* T is the Maximum delay time.

In this design there are 7 groups all using the RGB profile.  

```{r}
G <- 7
N <- 3  # four DMX channels for per group.

T <- 2.7 * ceiling(N*G/45)
```
