...
> The sconce lights also have an timed switch which is next to the keypad, if you want sconce control during the day you will need to manual turn them on as well.

 

The programming is managed via the app which communicates via bluetooth. There is a configuration menu item in the app. There are three items that each contribute to the configuration.
...
Control of the ADJ Ultrabar 12 devices presents a challenge. These are configured as 6 or 12 channels devices. This DMX footprint is not directly supported by the echo equipment.  We have assumed that we can tell the echo that these are four channel devices (RGBW) and skip the following 8 DMX addresses. This needs to be tested. (Simialr story for the UB6).

>Note: Any one device can only belong to one Zone.
...
> TODO: The Balcony Easy/West Bar DMX assignments need to be updated post Oct 2018 changes.
