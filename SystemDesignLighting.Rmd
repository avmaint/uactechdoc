...
balcony.refs <- tribble(
  ~group, ~name, ~type,        ~x,      ~y,      ~z,   ~notes,
  "balcony", "b1", "ref",       35,    -50,       0,  "",
  "balcony", "b2", "ref",       35,    -25,       0,  "",
  "balcony", "b3", "ref",       20,    -35,       0,  "",
  "balcony", "b4", "ref",       12,    -35,       0,  "",
  "balcony", "b5", "ref",        6,    -31,       0,  ""
)
...
```{r room, echo=FALSE}
  g <- ggplot(stage.refs) + coord_equal() +
  geom_point( aes(x=x, y=y)) + 
  geom_path(data=l1, aes(x=x, y=y) ) +
  geom_path(data=l2, aes(x=x, y=y) ) +
  geom_path(data=l1, aes(x=-x, y=y) ) +
  geom_path(data=l2, aes(x=-x, y=y) )  +
  geom_path(data=room.refs, aes(x=x, y=y)) +
  geom_path(data=room.refs, aes(x=-x, y=y))  
    
g <- g +   geom_text( aes(x=x, y=y, label=name), nudge_y = 2, label_size=.1) +
  geom_text(data=room.refs, aes(x=x, y=y, label=name), nudge_y = 2, label_size=.1)

channels <-  dmx.details %>% filter(!is.na(x))
gnew <- g + 
  geom_point(data=channels, aes(x=x, y=y, color=Type))  +
     geom_text(data=channels, aes(x=x, y=y, label=Channel), 
               nudge_x=channels$nx, nudge_y=channels$ny   ) 
