...
There is a simple lighting keypad located next to the auditorium doors by the main office (south east). It enables the house lights to set according to several preset looks, of which there are 8. (Terry can change these as needed. Details in the System Desgin Guide.) These presets operate like "radio buttons", that is, selecting one preset, deselects the previous preset. 

The OFF preset should be selected when the room is not in use.

A button turns blue when it is selected, orange otherwise.

The keypad and the main console work together. The controls are added together; brightest wins. This means than if either control says a light is on, it will be on. For a light to be off, both must say it is off. 

> as of Nov 6, these have not yet been programmed. 

The sconce lights also have an timed switch which is next to the keypad, if you want sconce control during the day you will need to manual turn them on as well. The timer is porgrammed to turn on the sconse during the evening hours (up to midnight), and Sunday morning.
...
#. Go to closet
#. Turn on power bar in closet.
#. Turn on big switch up high

![The switch in the red box](../../images/sol-BalconyClosetPower.png)

## Power Off

#. To turn off, reverse the order and turn off.

> When leaving the system for the day ensure that lower level keypad is set of "off" (The button will be blue). 
