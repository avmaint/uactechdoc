...
The connecting lines are illustrative not specifc. Some of those lines are implemented as analog connections, some are GigaAce, and some as Dante.
 
## Inputs

There are a variety of microphone inputs that come from the stage. 48 from the floor pockets, and an additional 16 via a stagebox.
...
There is also 128 dante inputs availalble. Those are used for the Shure Wireless Mics, the feed from ProPresenter and from the Livestream DAW.
...
Having a consistent wiring plan helps make system more predictable and easier for all audio engineers to use. The consistent wiring plan is also essentail to leveage the power of console's scene recall feature. It minimizes the amount of rewiring required. See @fig-sda-inputs-m7-map-a and @fig-sda-inputs-m7-map-d

The chart which follows is the standard input map if using *Scene 014*. 

```{r m7ina}
#| label: fig-sda-inputs-m7-map-a
#| fig-cap: "M7CL Inputs"
get_inputs("M7CL", "002") |>
	filter(is.na(NetworkDevice)) |>
	...
	gt() |>
	opt_stylize(style=3) |>
	tab_header("M7CL Input Map - Analog") 
```

```{r m7ind}
#| label: fig-sda-inputs-m7-map-d
#| fig-cap: "M7CL Inputs"
get_inputs("M7CL", "002") |>
	filter(!is.na(NetworkDevice)) |>
	gt() |>
...
As introduced in the earlier section there are two primary mix consoles in the auditorium System. Both of these consoles are digital with full scene store and recall. Recalling *scene 014* on the M7CL will cause both consoles to reset to our standard starting point which is an excellent foundation for 95% of the services we run in a year. 
...
There are some additional outboard components which complete the signal path. The main house output flows through a TC Electronics Finalizer, which provides multi-band compression. The output of that then goes two directions. one channel feeds the audio recording subsystem and the other feeds into the FOH DSP - a  BSS soundweb (ZAXU-0001).
...
node [shape = Mrecord style=filled , fillcolor="white:beige"  , fontsize = 10,
  ...
  subgraph cluster_rack { label="ZVHU-0005 rack"  
                              tooltip="Front desk Rack"
                               
        ZAAU0009 [label="{{Audio Hub|ZAAU-009}}", 
                      tooltip="Stuart"]
        24100800 [label="{{Dante Interface|2410-0800}}"]
        24100802 [label="{{DirectBox|2410-0802}}"]
        24100803 [label="{{GenderBender|2410-0803}}"]
  }
  
 subgraph cluster_mixrack {
 label="dLive MixRack"
 24073002 [label="{{dante interface|2407-3002|{<o67>67|<o68>68}}}"]
 } 

subgraph cluster_lobby { 
label="Lobby Ceiling"
ZAAU0010A  [label="Speaker with Amp|ZAAU-0010 A", tooltip="Speaker"]
ZAAU0010B  [label="Speaker with Amp|ZAAU-0010 B", tooltip="Speaker"]
ZAAU0010C  [label="Speaker with Amp|ZAAU-0010 C", tooltip="Speaker"]
ZAAU0010D  [label="Speaker with Amp|ZAAU-0010 D", tooltip="Speaker"]
}

24073002:o67 -- 24100800
24073002:o68 -- 24100800
 
24100800 -- 24100803
24100803 -- 24100802
24100802 -- ZAAU0009  [label="2410-0801"]
