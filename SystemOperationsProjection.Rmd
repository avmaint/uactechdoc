...
The best way to watch these videos is via the training postings on RightNowMedia. There are two modules one covers the basics, and the second more intermedite topics.
...
Series Folder
: This is for graphics which are to be used for an entire series. It contains the centre screen graphic, the lower 3rd graphics for video, the series Welcome graphic, plus other useful backgrounds.

> When we start a new series, we will need to update at least two things in ProPresetner: The "_Welcome Current Series" document and the "Series Graphic" in "Props".  
...
![click on the grey triangle to see the list of services](../../images/sop-newPCOplaylist3.jpg)
...
![Import](../../images/sop-ppt-import-2.png)

 

Once the import is complete, you can then link the new document to the playlist, and add the multiscreen and prop cues.

**If you see a white flash** as you transition to the first slide, change the background colour to black. The background colour is displayed first and then the graphic is brought in via the transition.
...
You can aslo do the PowerPoint Export compleltely outside of ProPresenter and export all slides as images, create a new ProPresenter document for them, and drag and drop all the saved JPGs into the document.
...
>Key point. Once the Stage Display Layout is set, it will not change until you set it to something else. This means you need to add the proper cues thorughout the playlist. For example, you need to set the worship lyric layout on the first song, The speaking layout for the message, and then the worship layout again one the first song after the message.
...
This video tutorial provides a good overview: https://youtu.be/ynqvjqTjv2M
...
Daniel Speaking
: Used when Pastor Daniel is speaking. It features the current slide with a small preview of the next slide. It also includes a clock, an elapsed time. Add timer cues to the first page of the message slides - reset timer (from cue pallete) and start timer. 
...
Click on the little blue ‘i’ beside the playlist document. This will pop the media properties window. Click on the properties button and then the behaviour setting to ‘Stop’. If ‘Stop’ is not selectable then you need to change the ‘Media Cue Behaviour’ to ‘foreground’.
...
![Media Properties](../../images/sop-MediaProperties.jpg)
...
Props
: A "Prop" is an extra graphic that can float on top of whatever else you have defined and can be turned on and off manually or added as a cue to a slide. The most common usage of Props will be to add the series graphic to the centre screen. There is a multi-screen cue setup for this you just need to add it to the first slide of the message. On the first Sunday of a new series we also need to change the series grpahic in the Props setting. It is also important to add 'Clear Props' cues after the message or any other time it has been turned on. There is "clear Prop" button in the toolbar (upper left) that will clear it if you forgot one.

Multi-screen Cues
: These cues control which content gets displayed where. There are two main variations. Tile and Stretch. Tile repeats the content across all three, and stretch makes the content span all three.
...
* Stage Display: "Pre Service Coutdown"
...
* Multiscreen: Choose "Song ABC Stretch" or "Song ABC Tile". The choice is influenced by how well it looks on the screen.  
    + Tile will repeat the graphic three times. Stretch will make one image stretch across all three. Try Stretch first.
    + If you pick ‘stretch’ and it isn’t wide enough, you will need to go into the media properties and change the option to “Stretch to Fill”. Sometimes it looks good to use "Scale of Fill".
...
* Props: Series Graphic
      + Don't forget to clear props on the next item after the message.
* Timer Cues as required (reset and start)    

One the last slide of the Message drop a 'Clear Props" cue.

[Back to top](#top)

## Transfering the ProPresenter Playlist to the Streaming Computer

> This doesn't not need to be done normally. The most common reasons to do this would be 1) a problem with the video link bewtween the computer and the ATEM, or a desire to have completely different formatting for the same content.

This process sets up the video streaming computer to mirror the main ProPresenter slide content. We do this becuase the main system is running at a triple wide resolution (5760x1080) and the stream is only single wide (1920x1080). 

#. First finailize the playlist on the main computer (CDMU-A001)
...
#. Navigate to --- need to decide on location --- ensure that the checkbox is selected to embed media
#. Move to the streaming computer, CDMU-A002
#. Open Propresenter
#. Go to File->Import->Import from File
#. Navigate to CDMU-A001 --- location from above --- and click OK (or Open)
#. You we likely see some pops ups about existing content. Select the 'apply to all' checkbox and choose 'Overwrite'
#. Move the newly import playlist to the top of the list.
#. Go thorugh each song and remove the background graphic and also apply the "UAC Lyrics" template. (Right click and "Remove" the graphic cue. Select the song item in the playlist and apply the "UAC Lyrics" Template either via right-click or the template icon at the top.)
#. Test that the streaming computer changes slides in-Sync with the main computer. 

> The connection between the two computers is defined on the Preferences⟶Network panel. The synchronization is communicated by playlist number, item number, and slide number, not by name. This is why we make the day's playlist, top of the list (#1 on both machines).
...
* Via the CueCommander application on CDWU-0009. On is on the 8:30 tab and off on the 11:30 tab.
...
## Dropping the OLD projection screen
...
The picture below shows the Video Matrix settings. The left had side shows the routing Matrix. THe inputs are along the top, the outputs down the side. A filled in circle indicates for each output where the input comes from.

For exmple, in the picture below, the Nursery (bottom row) is source from the ATEM Program output.

![Operating the Matrix switch via CueCommander](../../images/sop-matrixNormal.jpg)

There are some shortcuts on the right hand side. If you click all the buttons in the yellow box, everything will be set to normal.

__Via the Web Page__

Another way to operate the matric video switch (Asset Tag ZVKU-A001) is from its web page. There should be a shortcut to it on the computer desktop, or use 
...
Grey is the secondary monitor Background Colour. An easy way to verify the scenario is to open a Finder window and drag it onto a projector screen. If it appears there then you know the video path is working.  
