...
This document provides the key technical design documentation for the lighting systems at UAC.
...
-   For more sophisticated control, the primary lighting control console is a grandMA3 onPC command wing XT (2607-2500) located in the balcony. It is used for all lighting operations more sophisticated than can be accomplished via the keypad.

# Changes Expected in the Fall of 2026

```{r}
changes <- tribble(
  ~Change, ~Description,
  "sACN", "The control topology will be revamped to use sACN as the primary communication backbone.",
  "ETC Nodes", "ETC sACN-DMX Nodes will be installed at key points to convert sACN traffic to local DMX.",
  "Direct sACN Support", "Some fixtures will directly support sACN and will not require a local sACN-to-DMX node.",
  "Control Override", "The grandMA3 console will be able to override the Echo controller (keypad) using sACN priorities.",
  "Backup Control System", "A backup control system (onPC) will be provided in case the grandMA3 console is not available.",
  "New Fixtures (2026)", "New lighting fixtures will be added, including 7 Ayrton Diablo S, 2 Elation KL Cyc S, 5 Acme Tornados, 3 Portland P2 EVOs.",
  "New Fixtures (2027)", "Replacement of the house lighting fixtures. All pendants will be replaced with Acclaim Cycliner One Spectrum and some missing fixtures restored. Under Balcony replaced with ETC Navis Color Fixtures. Sconce light also likely replace with Acclaim Cycliner One Spectrum. The over stage posts will likely be replaced with LEDs on a relay."
  
)

changes |>
	gt() |>
	tab_header("Expected Changes in Fall 2026 and Beyond") |>
	opt_stylize(style=3)
```
...
* DMX, [an industry standard protocol](https://en.wikipedia.org/wiki/DMX512), is the backbone. We use only one DMX universe for output, driven directly from the DMX A port of the grandMA3 command wing
* ETC Echonet
* and KetraNet Mesh wireless

## sACN (Streaming Architecture for Control Networks)
In the fall of 2026 will will 
revamp the control topology and make sACN 
the primary communication backbone.
 
When sACN is implemented, 
the grandMA3 console will output all controls via 
sACN and will no longer directly emit DMX. 
ETC Nodes will be installed at key points that will convert the 
sACN traffic to local DMX.  

Some fixtures will directly support sACN and will not require a local sACN-to-DMX node. 

One key design objective is that i want to be able to leverage sACN priorities to allow the grandMA3 console to override the Echo controller (keypad) and also provide for a backup control system in case the grandMA3 console is not available.

This diagram (@fig-uac_sacn_lighting_170ch)  is illustrative of what this might look like. THe ETC DMX nodes are not shown. 

```{dot fig-uac_sacn_lighting_170ch}
//| file: gv/uac_sacn_lighting_170ch.gv
```


How these DMX systems were interconnected is depicted on the diagrams below. This will change in the fall of 2026. 
...
```{r dmx_control}
dmx_control <- c( 
		  "2607-2500"   # grandMA3 onPC command wing XT
		, "ZLIU-0003"   # opto-isolator / splitter
		, "2310-0220"   # Echo DMX scene controller
		, "2310-0211"   # ADJ 4-channel dimmer
		, "ZLIU-A004"   # UFMP8 relay panel
		 )
# Excluded devices are barriers for the walk. The console's LAN port and
# touchscreens would otherwise drag in the whole building network.
xd <- c("ZLKU-B002", "ZLLU-B054", "ZLLU-B052", "bt_node"
		, "NSCU-A004", "2607-1000", "2607-1001")
...
//| fig-cap: DMX Control Topology (generated from the cable database)
//| cap-location: bottom
...
//| fig-cap: Control Topology (simplified)
//| cap-location: bottom
...
It enables the house lights to be set according to several pre-defined looks.
...
* The sconce lights have a timed switch which is next to the keypad; if you want sconce control during the day you will need to manually turn them on as well.
...
* Control of the ADJ Ultrabar 12 devices on the walls presents a challenge as they are configured as 12 address devices and this DMX footprint is not supported by the *Echo* equipment.
...
```{r keypad}
#| label: tbl-keypad-index
#| tbl-cap: Keypad Index
keypad |>
  arrange(Button) |>
	gt() |>
	opt_stylize(style=3)
...
The detailed programming for each Button, Preset and Zone is shown in @fig-detailed-program1 and @fig-detailed-program2.
...
The channel IDs in these documents are the channel numbers as they were in the prior ETC ColourSource console. For the grandMA3 console, the fixture IDs are the old channel numbers plus 100 . The DMX addresses remain the same and are shown in the table below.

In the fall of 2026, this may all change.
...
2.  Via the grandMA3 console
    -   Located in the balcony on the rear desk.
    -   Many sophisticated looks can be programmed into the console.

These two control systems cannot be successfully used at the same time. When using the main console select the *Console Mode* button on the keypad.
...
:::{.callout-note}
Many of the Ketra lamps have failed and have been redeployed in order to maintain the best possible coverage with the remaining lamps. What is documented here is the original design and does not reflect the current state of the system.
:::
...
```{r groups   }
#| label: tbl-ketra-groups
#| tbl-cap: Ketra Groups
...
With all of them on, the centre section will be brighter than the outside.
...
The 10 centre pots are in one group to wash most of the platform.
...
```{r}
  ...
  mutate(Distance = Feet + Inches/12) |>
	mutate(DistanceMeters = round(Distance * 0.3048, 2)) |> 
  gt() |>
...
The grandMA3 console has a LAN connection (cable 2308-2600 to NSCU-A004 port 7).
RJ45 and cat6 cabling is also used for some DMX interconnects - in those cases the protocol is DMX, not Ethernet.

```{r net_dets, results="asis"}
db.network  |> 
	inner_join(db.assets, by=join_by(AssetTag == AssetTag)) |>  
  filter(Category=="Lighting") |> 
  select(Location, Usage.x, MAC, IP, URL, Notes) |>
	gt() |>
	opt_stylize(style=3) |>
