...
#. Turn on the power for the video mixer and the cameras. 
    * [Turning it on and off](#turning-it-on-and-off)
#. Ensure both memory cards in the recorders and formatted.
    * [Formatting a card](#formattting-a-card)
#. Run the "Do Setup pre-service" macro
    * [Standard Macros Defined for UAC](#standard-macros-defined-for-uac)
#. Get the Livestream running
    * [Livestreaming](#livestreaming)
#. Review the "Order of Service" for the morning and look for items that may require special attention and planning. 
    * Examples include: communion, baptism, solos, dramas, videos, etc.
    * You may choose to pre-define a camera position for that item
#. Check to see who is preaching this morning and make sure we have the correct Lower 3rd title loaded into media player 2
    * [Using the built-in Media Player](#using-the-built-in-media-player)
#. Fine tune the camera settings
    * [Camera Settings](#camera-settings)
#. By 9:30 ensure the lobby TV is on and at that time should be displaying the annoucement loop. By 10:00 it should be showing video mixer program.
    * Detailed instructions need to be written.
...
```{r fig.height=2}
img <- readJPEG("../../images/sov-CamRemote.jpg") 
...
#. Input #3
: A secondary feed from the presentation Switch. This can be used when the windows machine is being used for native PowerPoint, OBS, or Mediashout. It will also be useful when we implement three independent screens from ProPresenter, you will be able to choose between two of the three projected images.
...
>You can fire a macro from the software using the macro menu or from the ATEM by pressing the macro button follow by the number.

>TODO: Verify that these numbers are correct.
...
7. __Do__ Start of message logos and start recording
    1. Run  MP setup 22
    2. Run DSK setup 23
    ...
    7. Pause for 3s
    8. Remove DSK2
8. __Do__ end of message and stop recorder
...
We now stream our entire service to "FaceBook Live". This is how it works. The "Program" output from the ATEM is wired into the into of the "Web Presenter" box. Its output is feed via USB to the into of the computer. The computer sees that devices as a web cam.
...
* at about 9:30am, press the "Start Streaming" button. It will then change to be the "stop streaming" button. Verify that the connection is working in a couple of ways: 1) the streaming button changed from "Start Streaming" to "Stop Streaming", 2) in the bottom right of the OBS screen there is a small green square indicating that our bit rate is good. 
...
> TODO: Need to refine the instructinos for 10am. We need to incorporate the "We will be stating shortly" scene as well; perhaps combine it with the Date scene. 

[Back to top](#top)

#Camera Settings

The cameras revert to default settings when powered on. The defaults are not the best for out environment. We need to make some adjustments each day. 
...
![Camera settings](../../images/sov-camerasettings.png)
...
Take the SD memory card from (usually) recorder #2 and insert it into the back of the video mac computer. (The cards are currently being formatted for MacOS).
...
#. Right click on the audio track to get the menu and select "Clip Attributes", select "embedded Channel 2" for both tracks.
![Clip Attributes Menu](../../images/sov-edit-ClipAttributesMenu.png)
![Source Channel](../../images/sov-edit-SourceChannel.png)
...
Once the rendering is complete, you are ready to upload to Vimeo.

Check the resulting file size, it should be about 2.5GB, less is better. We have two ways to upload to Vimeo. The App method is easierto get it up quickly and doesn't require you to know the password.
